function mni_ch_table = getMNIcoords(digfile, SD)

% Last updated: Jun 8, 2026
% Updated by: YC, Jadyn

% MNI coordinates of 10-20 reference points

% OLD version of reference points:
% mni_refs = [2.32,62.34,-21.85,1;          % Nz
%             0.882,-13.4729,74.1371,1;     % Cz
%             -60.6623,-15.935,-34.9223,1;  % LPA
%             -4.77,-100.26,-14.39,1];      % Iz

% NEW version of reference points 
mni_refs = [2.32,62.34,-21.85,1;            % Nz
    61.99,-15.62,-34.4,1;                   % AR
    -60.66,-15.94,-34.92,1;                 % AL
    0.882,-13.4729,74.1371,1;               % CZ
    -4.77,-100.26,-14.39,1];                % IZ

numchannels = size(SD.MeasList,1)/2;
nSrc = SD.nSrcs;
nDet = SD.nDets;
src_det_pair = SD.MeasList(1:numchannels,1:2);

% Read the digpts file
digpts = readtable(digfile);
digpts = [table2array(digpts(:,2:4)), ones(size(digpts,1),1)];

% Extract the coordinates of the reference points
% CAUTION: Cross-check with your digpts.txt files to ensure you're
%          extracting the right rows
%          Here, it assumes that the first 5 rows are the reference points
digpts_refs = digpts(1:5,:);

% Compute transformation matrix using reference points
trsfm = mni_refs' * pinv(digpts_refs)';

% Apply transformation to all points
mni_pts = trsfm * digpts';
mni_pts = mni_pts';
mni_pts = mni_pts(:,1:3);

% Extract MNI coordinates of sources and detectors
% CAUTION: Cross-check with your digpts.txt files to ensure you're
%          extracting the right rows
%          Here, it assumes that the sources start from the 6th row
mni_srcPos = mni_pts(6:6+nSrc-1,:);
mni_detPos = mni_pts(6+nSrc:end,:);

% Find channel coordinates based on the coordinates of your sources and
% detectors
mni_ch = nan(numchannels,3);
for i=1:numchannels
    x = (mni_srcPos(src_det_pair(i,1),1) + mni_detPos(src_det_pair(i,2),1))/2;
    y = (mni_srcPos(src_det_pair(i,1),2) + mni_detPos(src_det_pair(i,2),2))/2;
    z = (mni_srcPos(src_det_pair(i,1),3) + mni_detPos(src_det_pair(i,2),3))/2;
    mni_ch(i,:) = [x y z];
end

mni_ch_table = array2table(mni_ch);
mni_ch_table.Properties.VariableNames = {'x','y','z'};

end
